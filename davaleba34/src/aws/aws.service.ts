import { Bucket$, GetObjectAclCommand, GetObjectCommand, PutObjectCommand, S3Client } from '@aws-sdk/client-s3';
...
export class AwsService {
    ...
    async uploadImage(filePath,file) {
        if(!filePath||!file){
            throw new BadRequestException("filePath and file are requiered fields")
        }
        const config = {
            Key: filePath,
